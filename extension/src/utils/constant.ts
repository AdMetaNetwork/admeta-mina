import browser from "webextension-polyfill"

export const WEB_TITLE = 'AdMeta x Mina'
export const WEB_DESCRIPTION = 'AdMeta x Mina'

// export const LINK_HTTP = 'https://mina.admeta.network/'
export const LINK_HTTP = 'http://localhost:3020'

export const API = 'https://api.admeta.network/'

export const RPC = 'https://eth-sepolia.g.alchemy.com/v2/QQF3gy6gGzyuaTGspit5vZ5mg08yXysu'
export const TEST_ACCOUNT = 'frozen captain slender cat multiply small okay cruise stone virus aim learn'

export const ADMETA_MSG_ACCOUNT = 'ADMETA_MSG_ACCOUNT'
export const ADMETA_MSG_SYNC_DATA = 'ADMETA_MSG_SYNC_DATA'
export const ADMETA_MSG_SYNC_DATA_BACK = 'ADMETA_MSG_SYNC_DATA_BACK'
export const ADMETA_MSG_UPDATE_SUCCESS = 'ADMETA_MSG_UPDATE_SUCCESS'
export const ADMETA_MSG_AD_PUSH = 'ADMETA_MSG_AD_PUSH'
export const ADMETA_MSG_ALL_SCORE = 'ADMETA_MSG_ALL_SCORE'
export const ADMETA_MSG_MINA_DEVELOP_ADDRESS = 'ADMETA_MSG_MINA_DEVELOP_ADDRESS'

export const REPORTING_TIME = 1000 * 10

export const PLAY_ICON = browser.runtime.getURL('images/play.png')
export const POW_IMG = browser.runtime.getURL('images/power.png')
export const CLOSE_IMG = browser.runtime.getURL('images/close.png')
