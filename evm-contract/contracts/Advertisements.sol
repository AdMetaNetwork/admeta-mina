...
contract Advertisements is Ownable, Initializable {
    ...
    function setUserLevel(uint level, uint256 score, string memory categoryScore, address user) external {

        bool userExists = false;
        for (uint i = 0; i < _usersLevel.length; i++) {
            if (_usersLevel[i].user == user) {
        ...
        if (!userExists) {
            _usersLevel.push(UserLevel(level, user, score, categoryScore));
        }
    }

    function getUserLevel(address user) external view returns(UserLevel memory) {
        ...
        require(userExists, "There is no user data");
